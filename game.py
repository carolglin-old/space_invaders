...
class LevelManager(CanvasObjects):
	def __init__(self):
		...
		self.win_music = '/Users/carollin/Dev/space_invaders/sounds/win.ogg'
	...
	def play_level_music(self):
		...
		if self.current == 3:
			self.play("background_music", self.win_music)
...
class Game(CanvasObjects, pg.sprite.Sprite):
	...
	def load_screen(self):
		...
		self.load_menu(self.start_message, 30, (250, 250), self.screen)
	...
	def remove_object(self, obj):
		...
		if type(obj) is Dude and self.status == "in_game":
			self.check_lives()
		if type(obj) is Alien:
			self.check_aliens()
			self.add_points(obj)
			self.try_special_object(obj)
	...
	def win(self):
		self.status = "win"
		self.remove_object(self.dude)
		self.level_manager.win_screen()

	def load_win_text(self):
		self.win_screen_text = ScreenText(self.win_message, (200, 10), self.screen, font_size = 30)

	def load_restart_text(self):
		self.restart_screen_text = ScreenText(self.restart_message, (500, 100), self.screen, font_size = 20)
	...
	def create_game_over_screen(self):
		...
		self.play("background_music", self.game_over_sound)
		self.prestart = True
	...
	def key_events(self):
		for event in pg.event.get():
			...
			if self.keys[pg.K_RIGHT]:
				self.dude.right()
			if self.keys[pg.K_SPACE] and (self.dude in self.dude_group):
				self.dude.shoot()
			if self.keys[pg.K_RETURN] and self.status == "win":
				self.restart()
	...
	def randomize_power_up(self, randomrange, obj):
		if random.randrange(randomrange) == 1:
			self.power_up = PowerUp(obj.x, obj.y)
	...
	def refresh_view(self):
		...
		if self.level_manager.current == 3:
			self.load_win_text()
			self.load_restart_text()

		pg.display.update()

class PowerUp(GameObjects, pg.sprite.Sprite):
	def __init__(self, x, y):
		GameObjects.__init__(self, 40, 40, x, y, '/Users/carollin/Dev/space_invaders/graphics/speed_powerup.png')
		pg.sprite.Sprite.__init__(self)
...
class Dude(GameObjects, pg.sprite.Sprite):
	...
	def invincible(self):
		if (time.clock() - self.spawn_time) <= 5:
			return True
		else:
			return False
