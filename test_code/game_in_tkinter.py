...
class CanvasObjects():
    ...
    def image_convert(self, imagepath, width, height):
        size = width, height
        graphic = Img.open(imagepath)
        graphic.thumbnail(size, Img.ANTIALIAS)
        resized = ImgTk.PhotoImage(graphic)
        return resized

    def darken(self, imagepath, width, height):
        print("darken")
        size = width, height
        graphic = Img.open(imagepath)
        graphic.point(lambda p: p * 0.3)
        graphic.thumbnail(size, Img.ANTIALIAS)
        graphic.show()
        resized = ImgTk.PhotoImage(graphic)
        return resized
...
class GameObjects(CanvasObjects):
    def __init__(self, width, height, img_source):
        super().__init__()
        self.width = width
        self.height = height
        ...
        self.graphic = self.image_convert(self.img_source, self.width, self.height)
...
class LevelManager(CanvasObjects):
    ...
    def load_block(self, raw, info_list):
        # taking the raw layout data into a list of dictionaries for each item
    ...
    def add_rows_columns(self, info_list, rows, columns, xcorner, ycorner, xspacing, yspacing):
        # unpacks the layout into a list of dictionaries with x,y coordinates
        index = 1
        while index <= rows:
    ...
    def movement_pattern(self, raw_movement_data):
        # add the alien movement patten to each dictionary
        movement_list = []
    ...
    def add_alien_info(self, raw):
        # adding additional alien information to the dictionaries
        for i in self.alien_info_list:
            if raw["id"] == "one":
                i.update(self.a.one)
            if raw["id"] == "two":
...
class Alien(GameObjects):
    def __init__(self, animation_graphic = '/Users/carollin/Dev/space_invaders/graphics/alienexplosion.png', animation_width = 320, animation_height = 40, num_sprites = 8, **kwargs):
    ...
    def hit(self, laser):
        ...
        if self.hit_points <= 0:
            ...
            self.graphic = self.darken(self.img_source, self.width, self.height)
